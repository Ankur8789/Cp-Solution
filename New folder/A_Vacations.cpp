...
ll f(ll i, ll contest, ll gym, vector<ll> &v)
{
    if (i == v.size())
        return 0;
    ...
    if (v[i] == 0)
        return dp[i][contest][gym] = 1 + f(i + 1, 0, 0, v);
    else if (v[i] == 1)
    {
        if (contest == 1)
            return dp[i][contest][gym] = 1 + f(i + 1, 0, 0, v);
        else
            return dp[i][contest][gym] = min(f(i + 1, 1, 0, v), 1 + f(i + 1, 0, 0, v));
    }
    else if (v[i] == 2)
    {
        if (gym == 1)
            ...
            return dp[i][contest][gym] = min(f(i + 1, 0, 1, v), 1 + f(i + 1, 0, 0, v));
    }
    else
    {
        ll ans = INT_MAX;
        if (contest == 0)
            ans = min(ans, f(i + 1, 1, 0, v));
        if (gym == 0)
            ans = min(ans, f(i + 1, 0, 1, v));

        ans = min(ans, 1 + f(i + 1, 0, 0, v));
        return dp[i][contest][gym] = ans;
    }
}
