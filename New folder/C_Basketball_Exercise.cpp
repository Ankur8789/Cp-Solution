...
//  }
ll t[100001][2][2];
 ll f(int i,vector<ll>& a,vector<ll>& b,ll r1,ll r2)
 {
    if(i>=a.size())
    return 0;
    if(t[i][r1][r2]!=-1)
    ...
    if(!r1 && !r2)
    {
       ans=max(a[i]+f(i+1,a,b,1,0),b[i]+f(i+1,a,b,0,1));
    }
    else if(r1==1 && r2==0)
    {
        ans=max(b[i]+f(i+1,a,b,0,1),f(i+1,a,b,0,0));
    }
    else
        ans=max(a[i]+f(i+1,a,b,1,0),f(i+1,a,b,0,0));
    return t[i][r1][r2]=ans;
 }
