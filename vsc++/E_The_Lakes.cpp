...
ll dx[] = {1, -1, 0, 0};
ll dy[] = {0, 0, -1, 1};
bool check(ll x, ll y, vector<vector<ll>> &v, vector<vector<ll>> &vis)
{
    if (x < 0 || y < 0 || x >= m || y >= n)
        return 0;
    if (v[x][y] == 0 || vis[x][y] == 1)
        return 0;
    return 1;
}
void dfs(ll i, ll j, vector<vector<ll>> &v, vector<vector<ll>> &vis)
{
    cnt += v[i][j];
    vis[i][j] = 1;
...
void solve()
{
    ...
    vector<vector<ll>> vis(m, vector<ll>(n));
    for (ll i = 0; i < m; i++)
    {
        for (ll j = 0; j < n; j++)
        {
