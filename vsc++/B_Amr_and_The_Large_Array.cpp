...
const ll M = 1e9 + 7;
const ll M_ = 1e7 + 10;
typedef priority_queue<ll> pq1;
typedef priority_queue<ll, vector<ll>, greater<ll>> pq2;
...
void solve()
{
    ...
    for (ll i = 0; i < n; i++)
        mp2[{v[i], mp[v[i]]}].push_back(i);
    ll l = -1, r = -1, ans = INT_MAX;
    for (auto t : mp2)
    {
        if (t.first.second == mx)
        {
            vector<ll> temp = t.second;
            ll sz = temp.size();
            if (temp[sz - 1] - temp[0] + 1 < ans)
            {
                ans = temp[sz - 1] - temp[0] + 1;
                l = temp[0];
                r = temp[sz - 1];
            }
        }
