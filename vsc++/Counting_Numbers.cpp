...
ll dp[20][2][10][2];
ll f(ll i, ll tight, ll last, string &s, int taken)
{
    if (i == s.size())
    {
        if (taken)
    ...
    if (dp[i][tight][last][taken] != -1)
        return dp[i][tight][last][taken];
    ll limit = 9;
    if (tight)
        limit = s[i] - '0';
    ll ans = 0;
    for (ll idx = 0; idx <= limit; idx++)
    {
        if (!taken)
        {
            if (idx == 0)
            {
                ans += f(i + 1, (tight & (idx == limit)), last, s, taken);
            }
            else
            {
                ans += f(i + 1, (tight & (idx == limit)), idx, s, taken ^ 1);
            }
        }
        else
        {
            if (idx != last)
            {
                ans += f(i + 1, (tight & (idx == limit)), idx, s, taken);
            }
        }
    }
    return dp[i][tight][last][taken] = ans;
}
void solve()
{
    ll a, b;
