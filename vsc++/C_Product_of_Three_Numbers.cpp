...
void solve()
{
    ...
    else
    {
        ...
        {
            cout << "YES" << ln;
            cout << pw(v[0].first, v[0].second) << " ";
            cout << pw(v[1].first, v[1].second) << " ";
            ll pr = 1;
            for (ll i = 2; i < v.size(); i++)
                pr *= pw(v[i].first, v[i].second);
            cout << pr << ln;
            return;
        }
        else
        {
            if (v.size() == 1)
            {
                ...
                else
                {
                    ...
                    cout << pw(v[0].first, 1) << " ";
                    cout << pw(v[0].first, 2) << " ";
                    cout << pw(v[0].first, v[0].second - 3) << ln;
                    return;
                }
            }
            else
            {
                ...
                else
                {
                    ...
                    cout << pw(v[1].first, 1) << " ";
                    ll pr = 1;
                    pr *= pw(v[0].first, v[0].second - 1);
                    pr *= pw(v[1].first, v[1].second - 1);
                    cout << pr << ln;
                }
            }
        }
