...
bool check(ll x, ll y, vector<string> &v, vector<vector<ll>> &vis)
{
...
void dfs(ll i, ll j, vector<string> &v, vector<vector<ll>> &vis, vector<pair<ll, ll>> &vp, ll t)
{
    if (t == 4)
    {
        if (v[i][j] == 'e')
    ...
    vis[i][j] = 1;
    vp.push_back({i, j});
    for (ll idx = 0; idx < 8; idx++)
    {
        ll nx = i + DIRS[idx][0];
        ll ny = j + DIRS[idx][1];
        if (check(nx, ny, v, vis) && v[nx][ny] == temp[t + 1])
        {
            dfs(nx, ny, v, vis, vp, t + 1);
        }
    }
    vp.pop_back();
    vis[i][j] = 0;
