...
ll dp[501][501][503];
ll f(ll i, ll last, vector<ll> &v, ll k)
{
    if (k == 0)
        return 0;
    if (k < 0)
    ...
    if (last == -1)
    {
        return dp[i][last+1][k] = max({f(i + 1, 0, v, k), v[i] + f(i + 1, 1, v, k - 1)});
    }
    else
    {
        ll ans = 0;
        for (ll idx = 0; idx <= last + 1; idx++)
            ans = max(ans, idx * v[i] + f(i + 1, idx, v, k - idx));
