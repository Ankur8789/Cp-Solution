...
bool f(ll i, ll idx, string &l, string &r, string &s, ll m, vector<vector<ll>> &dp)
{
    if (i == m)
        return false;
    if (dp[i][idx] != -1)
    ...
    {
        ll lb = lower_bound(mp[ch - '0'].begin(), mp[ch - '0'].end(), idx) - mp[ch - '0'].begin();
        if (lb == mp[ch - '0'].size())
            return dp[i][idx] = true;
        else
        {
            ans |= f(i + 1, mp[ch - '0'][lb] + 1, l, r, s, m, dp);
        }
    }
    return dp[i][idx] = ans;
}
