...
void dfs(ll x, vector<ll> &ideg, vector<ll> &vis, ll &comp, unordered_map<ll, vector<ll>> &adj, bool &hey)
{
    vis[x] = 1;
    comp++;
    if (ideg[x] == 1)
...
void solve()
{
    ...
    ll comp = 0;
    vector<ll> sofcomp, jud;
    vector<ll> vis(n + 1);
    for (ll i = 1; i <= n; i++)
    {
        if (vis[i] == 0)
        {
            dfs(i, ideg, vis, comp, adj, hey);
            sofcomp.push_back(comp);
            if (hey)
                jud.push_back(comp);
            hey = false;
            comp = 0;
    ...
    // for (ll i = 0; i < sofcomp.size(); i++)
    //     cout << sofcomp[i] << " ";
    // dbg;
    // for (ll i = 0; i < jud.size(); i++)
    //     cout << jud[i] << " ";
    ll mx = sofcomp.size();ll mn;
    if (jud.size() != 0)
         mn = sofcomp.size() - jud.size() + 1;
    else
    {
        mn = mx;
    }
