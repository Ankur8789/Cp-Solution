...
//  }
 int t[100001][2][2][2];
ll f(ll i,vector<vector<ll>>& v,ll a,ll b,ll c)
{
    if(i>=v.size())
    return 0;
    if(t[i][a][b][c]!=-1)
    ...
    ll ans=INT_MIN;
    if(a!=1)
    ans=max(ans,v[i][0]+f(i+1,v,1,0,0));
    if(b!=1)
    ans=max(ans,v[i][1]+f(i+1,v,0,1,0));
    if(c!=1)
    ans=max(ans,v[i][2]+f(i+1,v,0,0,1));

    return t[i][a][b][c]=ans;
