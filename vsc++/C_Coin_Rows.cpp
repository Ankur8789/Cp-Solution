...
void solve()
{
    ...
    vector<vector<ll>> ps1(2, vector<ll>(n));
    for (ll i = 0; i < 2; i++)
    {
        ll ps = 0;
        for (ll j = 0; j < n; j++)
    ...
    for (ll i = 0; i < n; i++)
    {
        if (i == 0)
        {
            if (ps1[0][i] + ps1[1][n - 1] > mx1)
            {
                mx1 = max(mx1, ps1[0][i] + ps1[1][n - 1]);
                idx = i;
            }
        }
        else
        {
            if (ps1[0][i] + (ps1[1][n - 1] - ps1[1][i - 1]) > mx1)
            {
                mx1 = max(mx1, ps1[0][i] + (ps1[1][n - 1] - ps1[1][i - 1]));
                idx = i;
            }
        }
    }
    ...
    for (ll i = 0; i < n; i++)
    {
        if (i == 0)
        {
            if (ps1[0][i] + ps1[1][n - 1] > ans)
            {
                ans = max(ans, ps1[0][i] + ps1[1][n - 1]);
                // idx = i;
            }
        }
        else
        {
            if (ps1[0][i] + (ps1[1][n - 1] - ps1[1][i - 1]) > ans)
            {
                ans = max(ans, ps1[0][i] + (ps1[1][n - 1] - ps1[1][i - 1]));
                // idx = i;
            }
        }
    }
