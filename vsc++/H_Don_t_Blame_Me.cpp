...
ll f(ll i, ll k, ll val, vector<ll> &v, vector<vector<ll>> &dp)
{
    if (i == v.size())
    {
        if (ppc(val) == k)
    ...
    if (dp[i][val] != -1)
        return dp[i][val];
    ll ans = 0;
    ans += f(i + 1, k, val, v, dp);
    ans %= mod;
    ans += f(i + 1, k, val & v[i], v, dp);
    ans %= mod;
    return dp[i][val] = ans;
}
void solve()
{
    ...
    vector<ll> v(n);
    for (ll i = 0; i < n; i++)
        cin >> v[i]; // memset(dp, -1, sizeof(dp));
    vector<vector<ll>> dp(n + 1, vector<ll>(128, -1));
    ll ans = f(0, k, 127, v, dp);
    cout << ans << ln;
}
